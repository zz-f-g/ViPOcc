...
class Monodepth2(nn.Module):
    ...
    def __init__(
        ...
        # decoder
        self.depth_decoder = Decoder(num_ch_enc=self.num_ch_enc, d_out=1, num_ch_dec=num_ch_dec, scales=self.scales)
    ...
    def forward(self, x, return_depth=False):
        ...
        if return_depth:
            res_inv_depth = self.depth_decoder(image_features)[("disp", 0)]
            return res_inv_depth
        else:
            feats = self.decoder(image_features)[("disp", 0)]
            return feats

...
from utils.plotting import draw_pred_maps, draw_img
...
def get_lidar_slices(point_clouds, velo_poses, y_range, y_res, max_dist):
    ...
    for y in ys:
        ...
        for pc, velo_pose in zip(point_clouds, velo_poses):
            ...
            slice_points_polar = torch.cat((torch.tensor(
                [[slice_points_polar[-1, 0] - math.pi * 2, slice_points_polar[-1, 1]]],
                device=slice_points_polar.device), slice_points_polar, torch.tensor(
                [[slice_points_polar[0, 0] + math.pi * 2, slice_points_polar[0, 1]]],
                device=slice_points_polar.device)), dim=0)
...
class BTSWrapper(nn.Module):
    def __init__(self, renderer, config, dataset) -> None:
        ...
        self.save_pred_gt_img = config.get("save_pred_gt_img", False)
        if self.save_pred_gt_img:
            self.save_dir = config.get("save_dir", "")
            if os.path.isdir(self.save_dir):
                self.save_dir = os.path.join(self.save_dir, f"predict_img_{self.z_range}")
            elif os.path.isfile(self.save_dir) and self.save_dir.endswith(".pt"):
                self.save_dir = os.path.join(os.path.dirname(self.save_dir), f"predict_img_{self.z_range}")
            else:
                raise ValueError('Invalid save dir.')
    ...
    def forward(self, data):
        ...
        if self.read_gt_occ_path == "":
            ...
            for id in range(id, min(id + self.aggregate_timesteps, seq_len)):
                points = np.fromfile(os.path.join(self.dataset.data_path, "data_3d_raw", seq, "velodyne_points", "data",
                                                  f"{self.dataset._img_ids[seq][id]:010d}.bin"),
                                     dtype=np.float32).reshape(-1, 4)
                points[:, 3] = 1.0
                points = torch.tensor(points, device=device)
                # NOTE also transfer velo coods to cam0
                velo_pose = world_transform.squeeze() @ torch.tensor(self.dataset._poses[seq][id],
                                                                     device=device) @ T_velo_to_pose
            ...
            occ_gt = np.load(os.path.join(self.read_gt_occ_path, name + "_occgt.npy"))  # [460,80]
        ...
        # 'self.read_gt_occ_path' specifies the path of pre-computed GT occupany. If not specified, generate GT Occ
        # from LIDAR points (a bit time-consuming for 300-frame accumulated gt).
        if self.read_gt_occ_path == "":
            slices = get_lidar_slices(points_all, velo_poses, self.y_range, yd,
        ...
        # ======================================== draw prediction maps ========================================
        if self.save_pred_gt_img:
            name = f"{seq.split('_')[-2]}_{self.dataset._img_ids[seq][id]:010d}"
            draw_pred_maps(is_occupied, img_name=f"{name}_gt", save_dir=self.save_dir)
            draw_pred_maps(is_occupied_pred, img_name=f"{name}_pred", save_dir=self.save_dir)
            draw_img(images[0, 0], img_name=f"{name}", save_dir=self.save_dir)
